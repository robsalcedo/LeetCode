...
class TreeNode {
    ...
    int helper(Query query, Node node){
        if(node.val == query.v){
            return countNodes(node, query.c);
        }
        for(Node child : node.children){
            int resTmp = helper(query,child);
            if(resTmp>0)return resTmp;
        }
        return 0;
    }

    int countNodes(Node node, char c){
        int res = 0;
        if('a' + node.val - 1 == (int)c)res++;
        for(Node child : node.children){
            res+= countNodes(child,c);
        }
        return res;
    }
    ...
    int[] countOfNodes(Node root, ArrayList<Query> queries, String s) {
        ...
        for(Query query : queries){
            if(s.indexOf(query.c)>-1){
                res[idx] = helper(query,root);
            }
            idx++;
        }
        return res;
